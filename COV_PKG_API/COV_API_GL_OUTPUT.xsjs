(function() {
	...
	// in the GL Output Tables.POST method is allowed           //
	...
	function _createEntries() {
		...
		if (oBody.JOURNAL.isArray) {
			for (var i = 0; i < oBody.JOURNAL.length; i++) {
				//Create the Batch Entry
				lvStatus = _createBatch(oBody.BATCH_HEADER, oBody.JOURNAL[i]);
			...
			if (oBody.JOURNAL) {
				//Create the Batch Entry
				lvStatus = _createBatch(oBody.BATCH_HEADER, oBody.JOURNAL);
	...
	function _createBatch(oBatchHeader, oJournal) {
		...
		try {
			...
			//Sender Id
			oStatement.setString(4, oBatchHeader.SENDER_ID);
			//Accounting Instance
	...
	function _createHeader(oJournal) {
		...
		try {
			...
			//Transaction Type
			oStatement.setString(9, oJournal.TXN_TYPE);
	...
	function _createItems(oJournal, oItems) {
		...
		try {
			...
			for (var i = 0; i < oItems.length; i++) {
				...
				//Posting Type
				oStatement.setString(8, oItems[i].POSTG_TYPE);
				//Posted Amount
				oStatement.setDecimal(9, oItems[i].POSTED_AMT);
				//Posted Currency
				oStatement.setString(10, oItems[i].POSTED_CCY);
				//FX Rate
				if (oItems[i].FX_RATE) {
